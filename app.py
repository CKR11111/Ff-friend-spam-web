...
# FINAL API CONFIG
FRIEND_API = "https://indrajit-free-all-w3ft.vercel.app/api"

def run_spam_in_background(accs, target):
    async def process():
        connector = aiohttp.TCPConnector(limit=100, ssl=False)
        async with aiohttp.ClientSession(connector=connector) as session:
            # 50 accounts ek saath bhejega (Super Fast)
            for i in range(0, len(accs), 50):
                ...
                for line in batch:
                    try:
                        u, p = line.split("|")
                        # action=friend_add, uid=SENDER, password=PASS, friend_uid=TARGET
                        url = f"{FRIEND_API}?action=friend_add&uid={u}&password={p}&friend_uid={target}"
...
def start_spam():
    try:
        ...
        if not accs:
            return jsonify({"error": "No IDs in bulk.txt"}), 400
